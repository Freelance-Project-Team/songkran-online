...
function WaterPlayScene({
	...
	const handleContinue = () => {
		...
		else if (isFace) setTimeout(onBack, 180);
	};
...
function SceneFrame(props: Parameters<typeof WaterPlayScene>[0]) {
	return (
		<div className="h-screen w-full flex items-center justify-center overflow-hidden bg-[#b8dff5]">
			<div className="relative h-full overflow-hidden" style={{ aspectRatio: '393 / 852' }}>
				<WaterPlayScene {...props} />
			</div>
		</div>
	);
}

function DesktopCanvas(props: Parameters<typeof WaterPlayScene>[0]) {
	return (
		<div className="flex h-screen">
			...
			<div className="w-110 shrink-0 flex flex-col items-center justify-center bg-white px-10 py-12 shadow-2xl overflow-y-auto z-10">
				...
				>
					{props.lang === 'th' ? 'เลือกตัวละครของคุณ' : 'CHOOSE YOUR CHARACTER'}
				</p>
			</div>
		</div>
	);
